...
- **Phase 5 — Hardening (post-v1):** _in progress._
  ...
  - **Accessibility (#4, Clarice-scoped):** messages list is an `aria-live="polite"` + `role="log"` region so new results are announced; the graph's `aria-label` is now the equation ("Graph of y = (x − 1)² + 2"), not "graph"; focus moves into the feedback screen on open and returns to the trigger on close; the info popover sets `aria-describedby`; visible `:focus-visible` rings on link-buttons/icon/CTA. Frontend **23 unit + 4 E2E**. (Carry-over: README install folds confirmed rendering on GitHub — 19 code boxes, 0 stray backticks.)
    - **Clarice #4 follow-up — focus-return on the splash path (fixed):** close-focus only worked from the chat-header trigger; `feedbackTriggerRef` lived on the chat button, which mounts only when a key is present, so closing the feedback view from the **key screen** focused null → dropped to `<body>`. Fix: forwarded a `feedbackButtonRef` to `ApiKeyScreen`'s "Send feedback" button; the close effect now focuses whichever trigger is mounted (`feedbackTriggerRef.current ?? keyFeedbackTriggerRef.current`). Added a splash-path focus-return test (the prior test set a key first, so only the chat path was exercised). No math/correctness impact.
...
- **Phase 5 (hardening) — APPROVED.** Property suite well-targeted; caught two genuine bugs (`y=x` guard, unhashable-`kind` 500), both re-verified by her. Answers: (1) `_guard` correct, no scope hole; (2) "valid envelope for any IR" is the right robustness invariant — keep it; (3) feedback-as-a-branch is fine for ≤3 views, add a router only when a 4th view or deep-linkable URLs appear (no dep now). Greenlit **#4 accessibility** with a prioritized scope (aria-live messages, equation as graph aria-label, overlay focus management, keyboard pass) — all implemented. Carry-over (README GitHub fold rendering) confirmed good.

- **Phase 5 #4 (accessibility, `7909491`) — APPROVED 🟡 with one follow-up (now resolved).** Re-ran at HEAD: 22 unit pass, tsc/eslint/prettier clean. Three a11y items verified clean (aria-live `role="log"` region; graph `aria-label` carries the real equation — she checked the renderer emits `title:{text}` at `graph_renderer.py:105`, not just the mocked test; info-popover `aria-describedby` + `:focus-visible` rings). **One gap:** focus-return on feedback-close was wired only for the chat-header trigger — the key-screen trigger had no ref, so the splash path dropped focus to `<body>`; untested because the focus-return test set a key first. **Resolved** (see Phase 5 #4 follow-up above) — ref forwarded to the key-screen button, close-effect focuses whichever trigger is mounted, splash-path test added; 23 unit pass. **Process nit (noted):** `7909491` folded her review text into `CLARICE.md` alongside a11y code — keep review-log edits in their own commits so the implement/review boundary stays clean. Going forward I leave `CLARICE.md` to Clarice and keep my fix commits single-concern.
